...
#include <assert.h>
__declspec(dllexport) const unsigned char* GetMD5(const char *password)
{
	static MD5 md5;
	assert(strlen(password)<1024);
	md5.init();
	md5.update((unsigned char*)password,strlen(password));
	md5.finalize();
	return md5._digest();
}

__declspec(dllexport) const char *GetMD5Str(const char *password)
{
	MD5 md5;
	assert(strlen(password)<1024);
	md5.init();
...
const unsigned char* md5_bin(const char *password)
{
	return GetMD5(password);
}

const char *md5_str(const char *password)
{

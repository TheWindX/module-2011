...
namespace ns_core
{
	namespace ns_debug
	{
		...
		//bool st_src_text::get_line_by_idx(u32 idx)
		//{
		//	//for(size_t i = 0; i<m_ln2pos.size(); ++i)//TODO
		//	return true;
		//}
		...
		u32 st_src_text::get_line_size()
		{
			return m_ln2pos.size();
		}
		...
		st_range st_debug_src_2_code::src2code(u32 src_idx)
		{
			st_range tmp(src_idx, src_idx+1);
			bool b = m_src2code.find(tmp);
			if(b)
			{
				return m_src2code.get_val();
			}
			else
			{
				return st_range();
		...
		void st_debug::init(const char* buff, u32 sz)
		{
			m_src_text.reset(buff, sz);
		}

		u32 st_debug::code2line_num(u32 code_idx)
		{
			st_range r = m_src2code.code2src(code_idx);
			u32 l1 = m_src_text.get_line_by_pos(r.m_left);
			return l1;
		...
		u32 st_debug::line2code_num(u32 line_idx, u32& left)
		{
			if(!m_src_text.get_line_range(line_idx) ) return U32_NA;
			u32 l_idx = m_src_text.get_left();
			u32 r_idx = m_src_text.get_right();
			st_range r1 = m_src2code.src2code(l_idx);
			st_range r2 = m_src2code.src2code(r_idx-1);
			left = r1.m_left;
			if(r2.is_NA() ) return r1.m_right;
			return r2.m_right;
			//u32 l1 = m_src_text.get_line_by_pos(r.m_left);
			//return l1;
		}

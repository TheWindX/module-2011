...
namespace ns_core
{
	...
	namespace ns_debug
	{
		...
		//这里的比较假设不会出现区域交叉的情况, 当然，一颗线段树各区域本身不会交叉
		...
		private:
			...
			ns_core::st_map<st_range, u32> m_pos2ln;

			st_range m_range;//temprorate use

			//bool get_line_range(u32 ln);
			//bool get_line_by_idx(u32 idx);
		};
		...
		struct st_debug_src_2_code
		{
			...
			st_range src2code(u32 src_idx);

		private://(pos_range, code_range)
			typedef ns_core::ns_debug::st_range_pair rp_t;
			ns_core::st_set<rp_t> m_src_code_pairs;

			ns_core::st_map<st_range, st_range> m_src2code;
			ns_core::st_map<st_range, st_range> m_code2src;
		};
		...
		struct st_debug
		{
			...
			void init(const char* buff, u32 sz);//设置 src_text
			u32 code2line_num(u32 code_idx);
			u32 line2code_num(u32 code_idx, u32& left);
		};
